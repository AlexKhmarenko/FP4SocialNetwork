...
public interface PostRepository extends JpaRepository<Post, Integer> {
  ...
  @Query(nativeQuery = true, value = "SELECT DISTINCT POSTS.P_ID, POSTS.PHOTO_FILE, POSTS.SENT_DATETIME, "
      ...
      + "ELSE 'false' "
      + "END) AS repostExists "
      + "FROM POSTS LEFT JOIN POST_LIKES on "
      + "POSTS.P_ID = POST_LIKES.POST_ID "
      + "INNER JOIN USERS ON POSTS.USER_ID = USERS.USER_ID "
  ...
  @Query(nativeQuery = true, value = "SELECT DISTINCT POSTS.P_ID, POSTS.PHOTO_FILE, POSTS.SENT_DATETIME,"
      ...
      + " REPOSTS R on POSTS.P_ID = R.POST_ID"
      + " JOIN USERS ON POSTS.USER_ID = USERS.USER_ID "
      + " WHERE R.USER_ID = :userId or  POSTS.USER_ID = :userId"
      + " ORDER BY JOINED_TIME DESC"
      + " OFFSET :offset ROWS FETCH NEXT :pageSize ROWS ONLY;")
  List<Object[]> findAllPostsAndRepostsByUserIdAsPostOneQuery(Integer userId, @Param("offset") int offset,
                                                              @Param("pageSize") int pageSize);
